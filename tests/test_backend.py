...
from backend.research import chunk_pdf,validate_claims,download_pdf,retrieve,WORKFLOWS
from tests.pdf_fixture import sample_pdf
import pytest

def register(client,email='researcher@example.org'):
    result=client.post('/api/auth/register',json={'name':'Alex Researcher','email':email,'password':'secure-test-pass'})
    assert result.status_code==200
    assert 'password' not in result.json()
    return result.json()
def project(client):
    register(client)
    result=client.post('/api/projects',json={'title':'Grounded language models','question':'What limitations affect retrieval augmented generation?'})
    assert result.status_code==201
    return result.json()['id']
def upload(client,pid):
    result=client.post(f'/api/projects/{pid}/upload',files={'file':('Research fixture.pdf',sample_pdf(),'application/pdf')})
...
def test_auth_session_and_logout(client):
    ...
    assert 'HttpOnly' in client.cookies.jar._cookies['testserver.local']['/']['researchos_session']._rest
    client.post('/api/auth/logout')
    assert client.get('/api/projects').status_code==401
    assert client.post('/api/auth/login',json={'email':'researcher@example.org','password':'wrong-password'}).status_code==401
    assert client.post('/api/auth/login',json={'email':'researcher@example.org','password':'secure-test-pass'}).status_code==200

def test_cross_user_isolation(client):
    pid=project(client); paper=upload(client,pid)
    with TestClient(app) as other:
        register(other,'other@example.org')
        for suffix in ('','/papers','/evidence','/messages','/jobs',f'/papers/{paper["id"]}/pdf'):
...
def test_upload_pdf_evidence_and_retrieval(client):
    ...
    assert client.get(f'/api/projects/{pid}/papers/{paper["id"]}/pdf').content.startswith(b'%PDF')

def test_invalid_pdf_rejected(client):
    pid=project(client)
    result=client.post(f'/api/projects/{pid}/upload',files={'file':('bad.pdf',b'not a pdf','application/pdf')})
    assert result.status_code==422
    assert client.get(f'/api/projects/{pid}/papers').json()==[]

def test_unsupported_question_abstains(client):
    pid=project(client); upload(client,pid)
    result=client.post(f'/api/projects/{pid}/chat',json={'content':'What quantum entanglement experiment was conducted?'})
    assert result.status_code==200
...
def test_no_key_returns_evidence_without_fabrication(client):
    pid=project(client); upload(client,pid)
    result=client.post(f'/api/projects/{pid}/chat',json={'content':'What English language limitations are reported?'})
...
def test_claims_require_exact_quotes():
    evidence=[{'id':'abc','text':'The study only evaluated English language documents.'}]
    good={'text':'English-only evaluation','sources':[{'id':'abc','quote':'only evaluated English language documents'}]}
    bad={'text':'Invented accuracy','sources':[{'id':'abc','quote':'The model achieved 99 percent accuracy.'}]}
    forged={'text':'Wrong source','sources':[{'id':'fake','quote':'only evaluated English language documents'}]}
...
def test_download_rejects_private_and_untrusted_urls():
    for url in ['http://127.0.0.1/admin','https://evil.example/paper.pdf','https://arxiv.org.evil.example/pdf/1','https://arxiv.org/admin']:
        with pytest.raises(ValueError): download_pdf(url)
...
def test_analysis_report_and_corpus_invalidation(client,monkeypatch):
    ...
    def fake_llm(system,content):
        import json
        data=json.loads(content); evidence=data['evidence']; e=next(e for e in evidence if e['section']=='Limitations')
        return {'claims':[{'text':'The study evaluates English documents only.','dimension':'Limitations','sources':[{'id':e['id'],'quote':e['text']}]}]}
    monkeypatch.setattr('backend.research.llm',fake_llm)
    with Session() as db:
        job=Job(project_id=pid,kind='analyze',status='running'); db.add(job); db.commit(); jid=job.id
    ...
    with Session() as db:
        running=db.get(Job,jid); running.status='completed'; db.commit()
    analysis=client.get(f'/api/projects/{pid}').json()['analysis']
    claim=analysis['papers'][0]['claims'][0]
    reviewed=client.patch(f'/api/projects/{pid}/review',json={'text':claim['text'],'source_ids':[s['id'] for s in claim['sources']],'status':'unsupported'})
    ...
    with Session() as db:
        j=db.get(Job,jid);j.status='completed';db.commit()
    assert client.patch(f'/api/projects/{pid}/papers/{paper["id"]}',json={'selected':False}).status_code==200
...
def test_guest_can_save_workspace_without_losing_projects(client):
    ...
    pid=client.post('/api/projects',json={'title':'Guest research','question':'What methods support retrieval augmented generation?'}).json()['id']
    claimed=client.post('/api/auth/claim',json={'name':'Saved Researcher','email':'saved@example.org','password':'safe-password-123'})
    assert claimed.status_code==200
    client.post('/api/auth/logout')
    assert client.post('/api/auth/login',json={'email':'saved@example.org','password':'safe-password-123'}).status_code==200
...
def test_full_workflow_uses_uploaded_corpus_and_honest_excerpt_mode(client):
    ...
    assert {e['stage'] for e in job['events']}=={'plan','discover','index','analyze','report'}
