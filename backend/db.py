...
class Chunk(Base):
    ...
    embedding: Mapped[list | None] = mapped_column(JSON().with_variant(Vector(), "postgresql"), nullable=True, default=lambda: None)
class Job(Base):
    __tablename__ = 'jobs'
    id: Mapped[str] = mapped_column(String(32), primary_key=True, default=uid)
    project_id: Mapped[str] = mapped_column(ForeignKey('projects.id'), index=True)
...
Index("one_active_job_per_project",Job.project_id,unique=True,sqlite_where=text("status IN ('queued', 'running')"),postgresql_where=text("status IN ('queued', 'running')"))
